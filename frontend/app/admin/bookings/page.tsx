...
import { getAdminBookingsAction } from "@/lib/actions/admin/admin-booking-action";
...
export default async function AdminBookingsPage({
  ...
  const result = await getAdminBookingsAction({ page, limit, search });
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#060914] px-5 py-8 text-white sm:px-8 lg:py-10">
      ...
      <div className="absolute right-[-180px] top-24 h-[28rem] w-[28rem] rounded-full bg-violet-500/10 blur-3xl" />
      <div className="relative mx-auto max-w-7xl">
        {result.success && result.data && result.meta ? (
          <BookingTable data={result.data} meta={result.meta} search={search} />
        ) : (
          <div className="rounded-[2rem] border border-red-400/30 bg-red-400/10 p-8 text-red-100">
            <h1 className="text-3xl font-black">Could not load bookings</h1>
